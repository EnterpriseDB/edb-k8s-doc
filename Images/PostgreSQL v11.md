...
```console
$ docker run --name edb-postgres -p 5432:5432 quay.io/edb/postgresql-11:latest bash -c '/police.sh && /launch.sh'
```
...
> The [Common Vulnerabilities and Exposures scan reports](https://quay.io/repository/edb/postgresql-11?tab=tags) contains a security report with all open CVEs.
...
Before deploying the EDB Postgres Docker Image, you must obtain [quay.io](http://quay.io) credentials to pull the prebuilt image from the [repository](https://quay.io/repository/edb/postgresql-11:latest).

```console
$ docker pull quay.io/edb/postgresql-11:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://quay.io/repository/edb/postgresql-11) in the Quay container registry.
...
$ docker run -it --rm \
    ...
    -d quay.io/edb/postgresql-11:latest bash -c '/police.sh && /launch.sh'
```
